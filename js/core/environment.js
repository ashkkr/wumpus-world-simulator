...
var Environment = function (i, j, width, height) {
    ...
    this.percept = function (player) {
        var i = player.getPosI();
        var j = player.getPosJ();

        var glitter = ArrayUtils.search(this.golds, [i, j]) ? true : false;
        var stench = false;
        var breeze = false;
        ...
        for (let k = 0; k < adjacents.length; k++) {
            ...
            if (posI < 0 || posI >= this.i || posJ < 0 || posJ >= this.j) {
                continue;
            }

            if (!stench && ArrayUtils.search(this.wumpus, [posI, posJ])) {
                stench = true;
            }

            if (!breeze && ArrayUtils.search(this.holes, [posI, posJ])) {
                breeze = true;
            }
